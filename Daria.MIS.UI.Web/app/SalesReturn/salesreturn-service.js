(function () {
    ...
    angular
        .module('salesreturnService', ['ngResource'])
        .factory('IndentBillDataService', IndentBillDataService);

    IndentBillDataService.$inject = ['$http', '$q'];

    function IndentBillDataService($http, $q) {
        var baseUrl = '/salesreturn/';
        return {
            ...
            getProductList: function (q) {
                ...
                var deferred = $q.defer();                
                $http({ method: 'GET', url: '/admin/GetProductsForUsageAutoComplete?q=' + q }).success(deferred.resolve).error(deferred.reject);
                return deferred;
            },
            ...
            getIndentInfo: function (userInfo) {

                var deferred = $q.defer();                
                $http({ method: 'POST', url: baseUrl + 'GetSalesItems', data: { no: userInfo } }).success(deferred.resolve).error(deferred.reject);
                return deferred.promise;
            },
