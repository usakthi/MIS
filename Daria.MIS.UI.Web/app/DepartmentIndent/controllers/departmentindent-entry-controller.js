BillEntryController.$inject = ['$scope', '$filter', '$route', '$routeParams', 'BillDataService', 'SupplierList', 'DepartmentList', 'ManufacturerList', 'UnitList', 'PharmacyIdEnc', '$location'];

function BillEntryController($scope, $filter, $route, $routeParams, BillDataService, suppList, deptList, mfgList, unitList, PharmacyIdEnc, $location) {
    ...
    $scope.purchaseHeader.Department = '';
    ...
    $scope.$watch('purchaseHeader.IPId', function (order) {
        if (order != 0) {
            var selected = $filter('filter')($scope.departments, { Id: order.RackId });
            $scope.purchaseHeader.Department = selected[order - 1].Department;
    ...
    $scope.showSelectedRack = function (order) {
        var selected = [];
        selected = $filter('filter')($scope.departments, { Id: order.RackId });
        return selected.length ? selected[0].Name : '';
    };
    ...
    $scope.onProductSelect = function ($item, $model, $label, $rowform, order) {
        ...
        var batchElement = $filter('filter')($rowform.$editables, { name: 'BatchNo' }, true)[idx];
        batchElement.scope.$data = $item.batchNo;

        var manfEditable = $filter('filter')($rowform.$editables, { name: 'Manufacturer' }, true)[idx];
        manfEditable.scope.$data = $item.manf;

        var manfIdEditable = $filter('filter')($rowform.$editables, { name: 'ManufacturerId' }, true)[idx];
        manfIdEditable.scope.$data = $item.manfId;

        var ExpiryElement = $filter('filter')($rowform.$editables, { name: 'ExpDate' }, true)[idx];
        ExpiryElement.scope.$data = $item.expDate;

        var StockEditable = $filter('filter')($rowform.$editables, { name: 'Stock' }, true)[idx];
        StockEditable.scope.$data = $item.stock;

        var GrnNoEditable = $filter('filter')($rowform.$editables, { name: 'GRNNo' }, true)[idx];
        GrnNoEditable.scope.$data = $item.grnNo;

        var PurDetIdEditable = $filter('filter')($rowform.$editables, { name: 'PurDetId' }, true)[idx];
