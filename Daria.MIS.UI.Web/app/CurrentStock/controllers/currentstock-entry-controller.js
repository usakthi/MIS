...
CurrentStockListController.$inject = ['$scope', '$filter', '$route', 'CurrentStockDataService', 'SupplierList', 'ManufacturerList', 'UnitList', 'PharmacyIdEnc', 'uiGridConstants'];

function CurrentStockListController($scope, $filter, $route, CurrentStockDataService, suppList, mfgList, unitList, PharmacyIdEnc, uiGridConstants) {
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
          ...
          ////},
          {
              name: 'S#', field: 'SNo', enableSorting: true, useExternalSorting: true
          },
          {
    ...
    $scope.delete = function (id) {        
        $('#confirm-purchase-delete').modal({ backdrop: false, keyboard: false })
            .on('click', '#delete', function () {
                CurrentStockDataService.deletePurchase(id)
                            .then(function (data) {
                                if (data.status === true) {
                                    $route.reload();
                                }
    ...
    $scope.search = function () {
        ...
        CurrentStockDataService.RptCurrentStock(params)
