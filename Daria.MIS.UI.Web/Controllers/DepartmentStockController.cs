...
{
    public class DepartmentStockController : Controller
    {
        private readonly IMasterDataService MasterDataService;
        private readonly IDepartmentStockService DepartmentStockService;
        private readonly IAccountService AccountService;

        public DepartmentStockController(IMasterDataService _masterSrv, IDepartmentStockService _purchaseSrv, IAccountService _accServ)
        {
            MasterDataService = _masterSrv;
            DepartmentStockService = _purchaseSrv;
        ...
        {
            User user = (User)Session["CurrentUser"];
            var model = DepartmentStockService.GetPurchaseDetails(id, user.PharmacyId);
        ...
        {
            User user = (User)Session["CurrentUser"];
            var model = DepartmentStockService.GetPurchaseList(user.PharmacyId);
            return Json(model);
        }

        [HttpPost]
        public ActionResult RptCurrentStock(DepartmentStockSearchDTO model)
        {
            User user = (User)Session["CurrentUser"];
            ...
            var records = DepartmentStockService.RptCurrentStock(model);
